...
function Panel() {
  ...
  return <div className="auth-card-wrap"><div className="auth-card"><span className="auth-heading__eyebrow">أمان حسابك</span><div className="auth-heading"><h2>تعيين كلمة مرور جديدة</h2><p>اختر كلمة مرور قوية لاستخدامها عند تسجيل الدخول.</p></div><form className="auth-form" onSubmit={submit}><label className="field"><span>كلمة المرور الجديدة</span><div className="field__input"><LockKeyhole size={17} /><input required minLength={8} type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="8 أحرف على الأقل" /></div></label><label className="field"><span>تأكيد كلمة المرور</span><div className="field__input"><LockKeyhole size={17} /><input required minLength={8} type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="أعد كتابة كلمة المرور" /></div></label><button className="button button--primary button--full" disabled={busy || !token}>{busy ? "جارٍ الحفظ..." : "حفظ كلمة المرور"}<ArrowRight size={17} /></button></form>{message && <div className="form-notice"><CheckCircle2 size={15} /> {message} <Link href="/login">تسجيل الدخول</Link></div>}{error && <div className="form-notice form-notice--error">{error}</div>}</div></div>;
}
