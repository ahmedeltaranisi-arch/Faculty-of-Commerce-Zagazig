...
export async function POST(request: Request) {
  try {
    ...
    const user = await prisma.$transaction(async (tx) => {
      const created = await tx.user.create({ data: { name: parsed.data.name, email, normalizedEmail: email, passwordHash, status: UserStatus.PENDING_VERIFICATION, roles: { create: { roleId: studentRole.id } }, enrollments: { create: { gradeId: grade.id, trackId: track.id } } }, select: { id: true, name: true, email: true, status: true } });
      await tx.emailVerificationToken.create({ data: { userId: created.id, tokenHash, expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000) } });
      return created;
    });
    try { await sendVerificationEmail(user.email, user.name, rawToken); } catch (emailError) { console.error("verification_email_error", emailError); }
    return ok({ user, message: "تم إنشاء الحساب. تحقق من بريدك الإلكتروني لتفعيل الحساب." }, request);
  } catch (error) {
